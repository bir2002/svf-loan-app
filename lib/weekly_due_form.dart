...
class WeeklyDueForm extends StatefulWidget {
  @override
  _WeeklyDueFormState createState() => _WeeklyDueFormState();
}

class _WeeklyDueFormState extends State<WeeklyDueForm> {
  ...
  Future<void> _fetchWeeklyLoans() async {
    ...
    if (response.statusCode == 200) {
      ...
      print("Failed to load weekly loans: ${response.body}");
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to load weekly loans. Please try again.')),
      );
    }
  }
  ...
  void _onLoanIdChanged(String? loanId) {
    ...
    final selectedLoan = weeklyLoans.firstWhere((loan) => loan['loan_id'].toString() == loanId);
  ...
  Future<void> _submitWeeklyDuePayment() async {
    ...
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final String? token = prefs.getString('token');
    final String apiUrl = "https://finance2024.ingenious-technologies.com/financeapp/public/api/executive/weekly-due-payment";
    ...
    if (response.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Weekly Due Paid Successfully')),
      );
    } else {
          ...
          child: ListView(
            children: <Widget>[
              DropdownButtonFormField<String>(
                ...
                items: weeklyLoans.map<DropdownMenuItem<String>>((loan) {
              ...
              SizedBox(height: 20),
              ElevatedButton(
                onPressed: _submitWeeklyDuePayment,
